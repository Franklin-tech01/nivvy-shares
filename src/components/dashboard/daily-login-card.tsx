...
import { useEffect, useState } from "react";
import { motion } from "framer-motion";
import { CalendarCheck, Check, Flame, Wallet } from "lucide-react";

import { Badge } from "@/components/ui/badge";
import { Card } from "@/components/ui/card";
import { cn, formatMoney } from "@/lib/utils";

const WELCOME_BONUS = 700;
const LOGIN_REWARD = 200;
...
const STORAGE_KEYS = {
	...
	lastLogin: "nivvy_last_login",
	streak: "nivvy_login_streak",
	welcomeBonusClaimed: "nivvy_welcome_bonus_claimed",
};
...
const getInitialState = (): RewardState => {
	...
	let balance = Number(localStorage.getItem(STORAGE_KEYS.balance) ?? 0);
	...
	const welcomeBonusClaimed =
		localStorage.getItem(STORAGE_KEYS.welcomeBonusClaimed) === "true";

	/*
	 * ==============================================
	 * ONE-TIME ₦700 WELCOME BONUS
	 * ==============================================
	 */
	...
	if (!welcomeBonusClaimed) {
		...
		localStorage.setItem(STORAGE_KEYS.welcomeBonusClaimed, "true");
	}

	/*
	 * ==============================================
	 * DAILY ₦200 LOGIN REWARD
	 * ==============================================
	 */
	...
	if (!loggedToday) {
		if (lastLogin) {
			const previousLogin = new Date(`${lastLogin}T00:00:00`);
			const currentDate = new Date(`${today}T00:00:00`);

			const daysSinceLogin = Math.floor(
				(currentDate.getTime() - previousLogin.getTime()) /
					(24 * 60 * 60 * 1000),
			);
...
export function DailyLoginCard() {
	...
	const { balance, streak, loggedToday, welcomeBonusClaimed } = state;
			...
			<Card className='overflow-hidden p-5'>
				...
				<div className='mt-5 rounded-md bg-muted/60 p-3'>
					<div className='flex items-center justify-between'>
						...
						<span className='tabular text-sm font-semibold'>
							{formatMoney(WELCOME_BONUS)}
						</span>
					</div>

					<p className='mt-1 text-xs text-muted-foreground'>
						Your ₦700 welcome bonus is credited once when you first sign in.
					</p>
				</div>
			</Card>
